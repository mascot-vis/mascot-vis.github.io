...
let coll = scn.repeat(line, dt, {attribute: "Topic", layout: msc.layout("grid", {numCols: 1, rowGap: 45})});
scn.classify(coll, {attribute: "Comparison", layout: msc.layout("grid", {numCols: 1, rowGap: 16})});

let textRow = scn.glyph(topic, rep, dem);
let textColl = scn.repeat(textRow, dt, {attribute: "Topic", layout: msc.layout("grid", {numCols: 1, rowGap: 40})});
scn.classify(textColl, {attribute: "Comparison", layout: msc.layout("grid", {numCols: 1, rowGap: 10})});
...
let enc = scn.encode(line.vertices[0], {attribute: "Republican", channel: "x"});
scn.encode(line.vertices[1], {attribute: "Democrat", channel: "x", shareScale: enc});
enc.rangeExtent = 400;

scn.encode(topic, {attribute: "Topic", channel: "text"});
scn.encode(rep, {attribute: "Republican", channel: "text"});
scn.encode(dem, {attribute: "Democrat", channel: "text"});
scn.encode(line, {attribute: "Comparison", channel: "strokeColor", mapping: {"Mentioned more by Republicans": "#f8ecf3", "Mentioned more by Democrats": "#e1f1fc"}});
