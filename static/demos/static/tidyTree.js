...
msc.encode(node, {attribute: "event", channel: "text"});
...
msc.encode(link, {channel: "strokeWidth", attribute: "child.value", rangeExtent: 12});

let lbl = scene.mark("text", {x: 100, y: 100, fontSize: "12px", fillColor: "#222"});
let lbls = msc.repeat(lbl, tree.linkTable);
msc.encode(lbl, {attribute: "child.value", channel: "text"});
...
//     let c = tree.getNode(l.dataScope.getAttrVal("child")),
//         p = tree.getNode(l.dataScope.getAttrVal("parent"));
