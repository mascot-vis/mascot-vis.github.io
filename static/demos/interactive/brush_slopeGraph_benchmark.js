let y1Range = [250, 500], y2Range = [200, 570], round = 0, y1Ext = 5, y2Ext = 5;
...
var run = function () {
    ...
    if (tg2.eventContext.get("yInterval")) {
        tg2.eventContext.set("yInterval", [y2, y2 + y2Ext]);
        scn.onChange("evtContext", tg2.eventContext);
    }
    ...
    if (y1 + y1Ext < y1Range[1]) {
        ...
        if (!tg2.eventContext.get("yInterval"))
            tg2.eventContext.set("yInterval", [y2, y2 + y1Ext])
        y2Ext += 2;
        requestAnimFrame(run);
    } else if (round < 5) {
        ...
        y1Ext = 2;
        y2Ext = 2;
        tg2.eventContext.set("yInterval", undefined);
        round++;
        requestAnimFrame(run);
    } else {
