...
| **sortChildrenByData**<br>(attribute, reverse, order) | sort the children by a data attribute<br>attribute (String): the data attribute to sort the children by<br>reverse (Boolean, optional): setting to true will sort in descending order;<br>default is false.<br>order (Array, optional): an array of attribute values in ascending order | void |
{.table-striped}