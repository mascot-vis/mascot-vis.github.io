...
The GridLayout class represents a layout that positions objects in a grid. To create a GridLayout object and apply it to a [collection](../../group/collection/), use the [_layout_ function](../../global/func/):
    ...
    let gl = msc.layout("grid", {numCols: 2, rowGap: 10});
...
|**rowGap**| the gap between adjacent rows in the grid | Number | 5 |
|**colGap**| the gap between adjacent columns in the grid | Number | 5 |
...
|**startCorner**| the corner from which the grid is filled | String | "topLeft" |
|**direction**| the direction in which the grid is filled | String | "rowFirst" |
{.table-striped}

### Methods
| method |  explanation   | return type |
| --- | --- | --- |
| **clone**() | returns a copy of this layout | [GridLayout](../grid/) |
{.table-striped}