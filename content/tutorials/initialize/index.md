...
To create a visualization, we need to [create a scene](../../docs/operations/create-element/#mscsceneparams) first:

```js
    let scene = msc.scene();
``` 

The returned object is an instance of the [Scene](../../docs/group/scene/) class, which is the top-level container in Mascot. Marks, glyphs, guides, and visual encodings are created through scene methods, while data-join and constraint operations are available as standalone `msc.*` functions.
...
```
To import a tree dataset, use the `msc.treeJSON` function:

```js
    let tree = await msc.treeJSON("path/to/file");
```

<span style="color:red;font-weight:bold">IMPORTANT</span>: The `msc.treeJSON` function is asynchronous and returns a promise, hence the await keyword. When the promise is fulfilled, a [Tree](../../docs/data/tree/) object is returned. You need to put the above line of code and any other code that handles the `tree` object in an asynchronous function, or use the [Promise.then()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then) method to handle the `tree` object.
...
```
To import a network dataset, use the `msc.graphJSON` function:

```js
    let network = await msc.graphJSON("path/to/file");
```

<span style="color:red;font-weight:bold">IMPORTANT</span>: The `msc.graphJSON` function is asynchronous and returns a promise, hence the await keyword. When the promise is fulfilled, a [Network](../../docs/data/network/) object is returned. You need to put the above line of code and any other code that handles the `network` object in an asynchronous function, or use the [Promise.then()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then) method to handle the `network` object.
