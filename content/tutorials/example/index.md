...
Next, we want to have multiple rectangles, each representing an age group. This is done by using the repeat operation:
    
    let collection = msc.repeat(rect, table, {attribute: "Age Group"});
...
Next, we want to further break down each age group by responses. We can use the divide operation:

    let {newMark, collection: bars} = msc.divide(rect, table, {attribute: "Response", orientation: "horizontal"});
...
Now we can map data attributes to visual channels. First, let's map "Percentage" to the width of the rectangles. We need to pass the new bar mark as an example to the encode method:
    ...
    scn.encode(newMark, {attribute: "Percentage", channel:"width"});
